  ...
  <article class="py-4 pb-16 my-4 border-b border-gray-400 last:border-b-0 flex flex-col md:flex-row justify-center">
    <div class="flex flex-col flex-1">
      <div>
        <ClientOnly>
          <div v-if="isUpcoming"
            class="block mb-2 transition duration-200 ease-in-out motion-reduce:transition-none fade-in">
      ...
      <div class="text-gray-700">
        <a :class="[talk.eventUrl && 'underline hover:no-underline inline-block hover:text-gray-800']"
      ...
      <ClientOnly>
        <a v-if="talk.isWorkshop && isUpcoming"
      ...
      <div class="flex md:flex-col mt-8">
        <a v-if="talk.videoUrl" :href="talk.videoUrl" :title="`Open video for ${talk.title}`"
          class="group flex items-center mr-8 md:mr-0" rel="noopener nofollow" target="_blank">
          <svg class="w-6 h-6 mr-2 group" role="presentation" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
            ...
            <path class="fill-current text-gray-800 group-hover:text-gray-700"
              d="M13.59 12l6.7-6.7A1 1 0 0 1 22 6v12a1 1 0 0 1-1.7.7L13.58 12z" />
            <rect class="fill-current text-gray-600 group-hover:text-gray-500" height="14" rx="2" width="14" x="2"
              y="5" />
          </svg>
          <span class="group-hover:underline text-lg">Video</span>
        </a>
        <a v-if="talk.slidesUrl" :class="talk.videoUrl && 'md:mt-6'" :href="talk.slidesUrl"
          :title="`Open slides for ${talk.title}`" class="group flex items-center" rel="noopener nofollow"
          target="_blank">
          <svg class="w-6 h-6 mr-2" role="presentation" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
            ...
            <path class="fill-current text-gray-600 group-hover:text-gray-500"
...
const formattedDate = computed(() => talkDate ? format(talkDate, 'do \'of\' MMMM yyyy') : '')
const isUpcoming = computed(() => talkDate ? isFuture(talkDate) : '')
</script>
